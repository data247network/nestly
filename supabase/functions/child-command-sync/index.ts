...
type Transport = "webview" | "native-lock"
type Body = { childId?: string; deviceSecret?: string; transport?: Transport; ack?: { id: string; status: AckStatus; result?: Record<string, unknown> }[] }
...
Deno.serve(async (req) => {
  ...
  const transport: Transport = body.transport === "native-lock" ? "native-lock" : "webview"
  ...
  let query = admin.from("device_commands")
    .select("id, command, payload, created_at")
    .eq("child_id", childId)
    .eq("status", "pending")
    .order("created_at", { ascending: true })
    .limit(10)
  if (transport === "native-lock") query = query.eq("command", "lock")
  else query = query.neq("command", "lock")

  const { data: pending, error } = await query
  if (error) return json({ ok: false, error: "command_read_failed" }, 500)
  const commands = []
  for (const item of pending ?? []) {
    const { data: claimed } = await admin.from("device_commands")
  ...
  return json({ ok: true, transport, commands })
})
