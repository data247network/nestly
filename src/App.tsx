...
export default function App() {
  ...
  if (!onboarded) {
    return <div className="safe-top flex h-full flex-col bg-white"><Onboarding index={card} onNext={() => setCard((c) => Math.min(2, c + 1) as 0 | 1 | 2)} /></div>
  }

  if (!role) {
    return <div className="safe-top flex h-full flex-col bg-white"><RoleGate /></div>
  }
  ...
  if (!signedIn) {
    return <div className="safe-top flex h-full flex-col bg-white"><Login onSignedIn={signIn} /></div>
  }
