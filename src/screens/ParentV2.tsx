...
export function ParentV2() {
  ...
  return <div className="flex h-full flex-col overflow-y-auto px-[22px] pb-8 pt-7">
    ...
    {tab==='safety' && <><Section title="Safety controls"><Quick label="Screen time" detail="Limits and routines" onClick={()=>go('screentime')} /><Quick label="School mode" detail="Emergency contacts stay available" onClick={()=>go('scenario')} /><Quick label="Emergency contacts" detail="Mum, Dad and trusted adults" onClick={()=>go('contacts')} /><Quick label="Web & app activity" detail="Filtering and reports" onClick={()=>go('activity')} /></Section><div className="rounded-2xl bg-tint p-4 text-xs text-tealInk"><b>Safety principle</b><p className="mt-1 leading-relaxed">Emergency communication should remain available even when routines or restrictions are active.</p></div></>}
