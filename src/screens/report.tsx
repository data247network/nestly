 ...
 * The report tab.
 *
 * Deliberately leads with coverage rather than totals. Over Bluetooth the
 * record is a backlog, so "nothing happened" and "the phones were never near
 * each other" look identical in the data — and only one of them is reassuring.
 * Every child's card states when it was last heard from, and the export carries
 * the same caveat.
 */
      ...
      buildReport(
        {
          ...
          lastSeenByChild: Object.fromEntries(
            live.map((c) => [c.deviceId, c.lastSeenAt ?? null]),
          ),
        },
        range,
      ),
    [state.children, state.activity, state.alerts, state.usageByChild, live, range],
  )
    ...
    <div className="flex h-full flex-col gap-4 overflow-y-auto px-[22px] py-[26px]">
      ...
      <div className="no-scrollbar -mx-[22px] flex gap-2 overflow-x-auto px-[22px]">
        {RANGES.map((r) => (
          <Pill
            ...
            onClick={() => dispatch({ type: 'setReportRange', range: r.id })}
          >
            {r.label}
          </Pill>
        ))}
      ...
      {report.sections.length === 0 ? (
        <div className="rounded-2xl bg-cream px-4 py-8 text-center text-[12.5px] text-body">
          No paired children yet. Pair a phone from the Device tab and their
          activity will collect here.
        </div>
      ) : null}
      ...
      {report.sections.map((s) => (
        <section key={s.child.id} className="rounded-2xl bg-cream p-4">
          <div className="mb-3 flex items-center gap-2.5">
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="text-[11px] text-body">
                {s.lastSeenAt
                  ? `Last heard from ${new Date(s.lastSeenAt).toLocaleString()}`
          ...
          {s.apps.length > 0 ? (
            <div className="mt-3.5">
              ...
              <div className="flex flex-col gap-1.5">
                {s.apps.slice(0, 5).map((a) => (
                  <div key={a.pkg} className="flex justify-between text-[12px]">
                    <span className="truncate">{a.label}</span>
                    <span className="shrink-0 pl-2 text-body">{fmtDuration(a.minutes)}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          {s.events.length === 0 ? (
            ...
            <div className="mt-3 text-[11.5px] text-muted">
              {s.events.length} recorded events · {s.alerts.length} alerts
            </div>
          )}
        </section>
      ))}
      ...
      {result ? (
        <div className="rounded-xl bg-tint px-3.5 py-3 text-[12px] text-tealInk">{result}</div>
      ) : null}

      <div className="flex-1" />

      <div className="flex gap-2.5">
        <GhostButton onClick={() => void exportAs('csv')}>
          {busy === 'csv' ? 'Exporting…' : 'Export CSV'}
        </GhostButton>
        <PrimaryButton onClick={() => void exportAs('pdf')}>
          {busy === 'pdf' ? 'Exporting…' : 'Export PDF'}
        </PrimaryButton>
      </div>
      <p className="text-center text-[10.5px] leading-relaxed text-muted">
        Exports cover only what has synced to this phone. A quiet period may mean
        the phones were simply apart.
      </p>
    </div>
  )
}
...
function Stat({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-xl bg-white px-3 py-2.5">
      <Display className="text-[17px] leading-none">{value}</Display>
      <div className="mt-1 text-[10.5px] text-body">{label}</div>
    </div>
  )
}
