...
import { ProtectionSetup } from './child'
...
export function ChildV2Home() {
  ...
  return <div className="flex h-full flex-col overflow-y-auto bg-parchment px-[22px] pb-8 pt-7">
    ...
    <div className="mt-4"><ProtectionSetup /></div>
    ...
    <div className="mt-5 grid gap-3"><ChildAction title="My Tasks" detail="Finish a task to earn extra time." onClick={()=>go('childChores')} /><ChildAction title="My routines" detail="See the routines affecting your device." onClick={()=>go('childRoutines')} /><ChildAction title="My requests" detail="Ask for time, access or a temporary exception." onClick={()=>go('childRequests')} /><ChildAction title="My rewards" detail="See approved rewards and extra time." onClick={()=>go('childRewards')} /><ChildAction title="Message hub" detail="Send and read notes with your parent." onClick={()=>go('childHub')} /></div>
